...
<form>
    ...
    <input type="hidden" name="monthContainer" id="monthContainer"  value="<?php echo date("M");?>" />
</form>
<script>
	var modal;
...
function changeactivity()
{
	...
	//18 Feb 2013 – 24 Feb 2013
		...
		function(){						
			$(".dhx_cal_next_button").click(function(){
				scheduleForNextPrev('next');
			});
			
			$(".dhx_cal_prev_button").click(function(){
				//scheduler.load("<?php //echo urlFormat('/index.php/calendarv2/getactivities') ?>?from=2012-09-01&to=2012-12-31");				
				scheduleForNextPrev('prev');
	...
	function scheduleForNextPrev(mode)
	{
			var preMonth = $("#monthContainer").val();
			var dateRange = $(".dhx_cal_date").html();
			var dateRangeArray = dateRange.split(" ");
			
			if(dateRangeArray.length == 2)
				currentMonth = dateRangeArray[0];
			else
				currentMonth = dateRangeArray[1];
			
			//console.log(mode+":"+preMonth+" "+currentMonth);	
			if(mode == 'currentMonth') {
				setTimeout(showLoader,1000);

				$('.dhx_cal_event').each(function(){
					scheduler.deleteEvent($(this).attr('event_id')); 
				});
			}
			
			
			if(preMonth != currentMonth || mode == 'currentMonth') {
				
					$.ajax({					
					type:"GET",
					url:"<?php echo urlFormat('/index.php/proajax/calendar/syncgcal') ?>?mode="+mode+"&month="+currentMonth+"",
					data: '',						
					success: function( html ){	
						var r = eval( '('+html+')' );
						...
						//console.log( r.result +" "+ mode +" "+ currentMonth);	
						scheduler.load("<?php echo urlFormat('/index.php/calendarv2/getactivities') ?>?mode="+mode+"&month="+currentMonth+"");
						$("#monthContainer").val(currentMonth);
	...
	showLoader = function () {
		if( $(".simplemodal-wrap").length == "" ) {
		modeload = $("#loading_msg").modal();
 			$("#message_txt").html( 'Loading' )			
			$("#loading_msg").html( $("#message_screen").html() )
		}	
	}
	
</script>
<div id="loading_msg" style="display:none;height:50px;width:140px;">	
</div>
